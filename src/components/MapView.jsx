...
function Heatmap({ data }) {
  ...
  useEffect(() => {
    ...
    const points = data.map((b) => [b.lat, b.lng, b.intensity || 1]);
...
export default function MapView() {
  ...
  const [addTab, setAddTab] = useState("rightclick");
  ...
  async function refreshBuildings(force = false) {
    if (buildingsRequestRef.current && !force) {
      return buildingsRequestRef.current;
    }
  ...
  useEffect(() => {
    ...
    const highBuildings = buildings.filter((b) => (b.high_count || 0) > 0);
    if (!highBuildings.length) return;

    const bounds = L.latLngBounds(
      highBuildings.map((b) => [Number(b.lat), Number(b.lng)])
    );
  ...
  useEffect(() => {
    ...
    const buildingId =
      searchParams.get("building") || searchParams.get("buildingId");
      ...
      <div className="map-wrapper">
        ...
        >
          ...
          <div style={{ fontSize: 10, lineHeight: 1.35 }}>
            CityHelp показывает обращения жителей по домам: жалобы, статусы,
            аналитика и соседская помощь. Используйте карту, чтобы видеть
            ситуацию в районе и добавлять обращения.
          </div>
        </div>

        <h1 className="sr-only">
          CityHelp — карта обращений по домам Минска и Беларуси
        </h1>
        <p className="sr-only">
          Жалобы и проблемы жильцов, статусы домов, аналитика по районам и
          соседская помощь и взаимопомощь. Добавляйте обращения и смотрите
          ситуацию на карте, помогайте другим оценить ситуацию на районе.
        </p>
        ...
        >
          {selectedBuilding && <div className="map-overlay" />}
            ...
            {buildings
              ...
              .map((b) => (
                <Marker
                  ...
                  opacity={
                    selectedBuilding && selectedBuilding.id !== b.id ? 0.7 : 1
                  }
                  ref={(marker) => {
                    if (marker) {
        ...
        {!addMode ? (
          <button onClick={startAddMode} className="add-marker-btn">
            Добавить метку
          </button>
        ) : (
          <div className="status-filter">
            ...
            <div style={{ fontSize: 13, lineHeight: 1.4 }}>
              Правый клик по карте (ПКМ) — запомнит координаты. Потом внизу нажми
              «Создать».
              <br />
          ...
          >
            ...
            >
              ...
              <div style={{ marginTop: 12, fontSize: 12, color: "#666" }}>
                * Можно оставить и правый клик, и добавление по адресу. По адресу
                метка появится после подтверждения — и сразу откроются жалобы.
              </div>
            </div>
          </div>
        )}
